...
import { useCreateServiceStationMutation } from '../../features/serviceStation/serviceStationSlice'
...
function AddServiceStation() {
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      await createServiceStation(formData).unwrap()
      message.success('СТО создана и отправлена на модерацию')
      navigate(backPath)
    } catch {
      message.error('Не удалось создать СТО')
    }
  }

  return (
    <div className="container">
      <Typography.Title level={2}>Добавить СТО</Typography.Title>
      <Card>
