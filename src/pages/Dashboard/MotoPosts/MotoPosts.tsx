...
function DashboardMotoPosts() {
  const { message } = AntdApp.useApp()
  const [pagination, setPagination] = useState({ page: 1, per_page: 10 })
  const { data, isLoading } = useGetDashboardMotoPostListQuery({ pagination })
  const [deleteMotoPost, { isLoading: isDeleting }] = useDeleteMotoPostMutation()

  const handleDeleteMotoPost = async (motoPostId: number) => {
    try {
      await deleteMotoPost(motoPostId).unwrap()
      message.success('Мото-пост удален')
    } catch {
      message.error('Не удалось удалить мото-пост')
    }
  }
  ...
  const columns: ColumnsType<MotoPost> = [
    ...
    },
    {
      ...
      render: (_value, record) => (
        <Space>
          <Link to={`/dashboard/moto-posts/${record.id}/edit`}>
            <Button icon={<EditOutlined />} size="small" />
          </Link>
          <Popconfirm
            title="Удалить мото-пост?"
            description="Действие нельзя отменить"
            okText="Удалить"
            cancelText="Отмена"
            okButtonProps={{ danger: true, loading: isDeleting }}
            onConfirm={() => handleDeleteMotoPost(record.id)}
          >
  ...
  return (
    <Card size="small">
      <Space style={{ width: '100%', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <Link to="/dashboard/moto-posts/create">
...
export default DashboardMotoPosts
